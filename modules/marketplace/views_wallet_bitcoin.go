...
func (c *Context) BitcoinWalletSendStep2GET(w web.ResponseWriter, r *web.Request) {
	...
	if withdraw {
		c.Amount = c.UserBitcoinBalance.Balance // util.RoundPlus(math.Max(c.UserBitcoinCashBalance.Balance-c.BCHFee, 0.0), 8)
	}
...
func (c *Context) BitcoinWalletSendStep2POST(w web.ResponseWriter, r *web.Request) {
	...
	c.BTCPaymentResult, err = receipt.BTCPaymentResult()
