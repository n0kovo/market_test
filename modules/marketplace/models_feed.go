...
func (itms FeedItems) PopulateRelations() FeedItems {
	...
	for i, _ := range itms {
		...
		case "staff_support_reply":
			user, err := FindUserByUuid(itms[i].ForeignUuid, false)
			if err != nil {
				continue
			}
			itms[i].OtherUser = *user
			supportThread, err := GetSupportThread(itms[i].OtherUser, false)
			if err != nil {
				continue
			}
			itms[i].Thread = *supportThread
...
func CacheFillPublicFeedItems() FeedItems {
	...
	gc.Set("public-feed-items", populatedItems)
	return populatedItems
}
...
func CacheGetPublicFeedItems() FeedItems {
	...
	if cFeedItems, _ := gc.Get("public-feed-items"); cFeedItems == nil {
...
func CacheFillStaffFeedItems() FeedItems {
	...
	populatedItems := feedItems.PopulateRelations()
	gc.Set("staff-feed-items", populatedItems)
	return populatedItems
}
...
func CacheGetStaffFeedItems() FeedItems {
	...
	if cFeedItems, _ := gc.Get("staff-feed-items"); cFeedItems == nil {
