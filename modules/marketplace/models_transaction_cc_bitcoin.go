...
func (bt BitcoinTransaction) UpdateTransactionStatus(t Transaction) error {
	...
	if t.CurrentPaymentStatus() == "COMPLETED" && newAmount.Balance == 0.0 {
		return t.SetTransactionStatus(
			"PENDING",
			newAmount.Balance,
			"Transaction status changed to PENDING",
			"",
			nil,
		)
	}

	if t.CurrentPaymentStatus() == "PENDING" && (bt.Amount-newAmount.Balance) <= bt.Amount*0.1 {
		return t.SetTransactionStatus(
			"COMPLETED",
			newAmount.Balance,
			"Transaction funded",
	...
	if t.IsPending() || t.IsFailed() {
		if (bt.Amount - newAmount.Balance) <= bt.Amount*0.1 {
			return t.SetTransactionStatus(
				"COMPLETED",
				newAmount.Balance,
				"Transaction funded",
...
func (bt BitcoinTransaction) Release(t Transaction, comment, userUuid string) error {
	...
	if vendor.Bitcoin == "" { // if seller doesn't have auto-withrawal wallet set up
