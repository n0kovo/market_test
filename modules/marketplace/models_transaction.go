...
type Transaction struct {
	...
	BitcoinTransactionUuid     string `json:"bitcoin_transcation_uuid" sql:"index"`
	BitcoinCashTransactionUuid string `json:"bitcoin_cash_transcation_uuid" sql:"index"`
	EthereumTransactionUuid    string `json:"ethereum_transcation_uuid" sql:"index"`
	PackageUuid                string `json:"package_uuid" sql:"index"`
	SellerUuid                 string `json:"seller_uuid" sql:"index"`
	BuyerUuid                  string `json:"buyer_uuid" sql:"index"`
	...
	BitcoinTransaction     BitcoinTransaction     `json:"bitcoin_transcation"`
	EthereumTransaction    EthereumTransaction    `json:"ethereum_transcation"`
	BitcoinCashTransaction BitcoinCashTransaction `json:"bitcoin_cash_transcation"`
}
...
type TransactionStatus struct {
	...
	PaymentReceiptUuid string    `json:"payment_receipt_uuid" sql:"index"`
...
type ViewTransactionStatus struct {
	...
	BTCPaymentResult  apis.BTCPaymentResult
	BCHPaymentResult  apis.BCHPaymentResult
	ETHPaymentResults []apis.ETHPaymentResult
}
...
func (t Transaction) CommissionPercent() float64 {
	if t.Seller.PremiumPlus {
		return MARKETPLACE_SETTINGS.PremiumPlusCommission
	}
	if t.Seller.Premium {
		return MARKETPLACE_SETTINGS.PremiumCommission
	}
	return MARKETPLACE_SETTINGS.FreeCommission
}
...
func (t *Transaction) SetTransactionStatus(
	...
	EventTransactionStatusChange(*t)
	return t.Save()
}
...
func CreateTransactionForPackage(
	...
	EventNewTransaction(transaction)
